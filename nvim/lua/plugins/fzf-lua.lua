return {
	...
	config = function()
		...
		vim.keymap.set("n", "<leader>/", builtin.lgrep_curbuf, { desc = "[S]earch Current Buffer" })
		vim.keymap.set("n", "<leader>sg", builtin.live_grep_native, { desc = "[S]earch by [G]rep" })
		vim.keymap.set("n", "<leader>q", builtin.diagnostics_document, { desc = "Open diagnostic [Q]uickfix list" })
